...
import EllipsisTypography from "@app/common/components/EllipsisTypography";
import { useEffect, useRef, useState } from "react";
import { CompleteStatus, Progress, FileUploadManager } from "../api/upload";
import { convertSeconds, getFormattedFileSize, getFormattedTime } from "@app/utils/utils";
        ...
        <Paper elevation={5} sx={{ marginTop: 2 }}>
            <UploadProgressItemGridBox>
                <Box sx={{
                    ...
                    <EllipsisTypography sx={{textAlign: "start"}}>{props.fileUpload.file.name}</EllipsisTypography>
                    <Tooltip placement="top" title={t("expected_remaining_time")}>
                        <Typography sx={{textAlign: "end"}}>{timeText}</Typography>
                    </Tooltip>
                </Box>
                ...
                <Box sx={{ gridRow: "3/4", gridColumn: "1/3", display: "flex", alignItems: "center", gap: 1 }}>
                    <LinearProgress
                        ...
                        sx={{ width: "100%" }}
                    />
                    <Typography sx={{width: "fit-content", textAlign: "end"}}>{(function () {
                        switch (progress.completeStatus) {
                            case CompleteStatus.IN_PROGRESS:
